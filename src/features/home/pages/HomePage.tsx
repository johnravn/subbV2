...
import { Eye, GoogleDocs, Message, RssFeed, Wallet } from 'iconoir-react'
  ...
  const { data: accountingConfig, isLoading: accountingLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        ...
        .select('accounting_software, accounting_api_key_encrypted')
...
function LatestSection({
  ...
  const formatActivityTitle = (activity: ActivityFeedItem): string => {
    ...
    switch (activity.activity_type) {
      ...
      case 'job_created':
        return `Created job "${metadata.job_title || activity.title || 'job'}"`
      case 'job_deleted':
        return `Deleted job "${metadata.job_title || activity.title || 'job'}"`
      case 'announcement':
